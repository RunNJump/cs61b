...
class CommandInterpreter {
    ...
    void insertStatement() {
        ...
        while (true) {
            ...
            if (values.length != table.columns()) {
                skipCommand();
                throw new DBException("Length is not matched");
            }
            table.add(values);
            if (!_input.nextIf(",")) {
                break;
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            ...
            return table1.select(table2, columns,
                    conditionClause(table1, table2));
        } else {
            return table1.select(columns, conditionClause(table1));
        }
    }
